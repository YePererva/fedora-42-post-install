...
```bash
# Might need to use a testing repo if not a recent is in active repo
sudo dnf --enablerepo=updates-testing install -y kicad kicad-packages3d kicad-doc
```

Alternatively, consider install with Flatpak:
...
ONLY if want to use it offline. Go to EadyEDA, create the account and login.
...
For BambuLab Studio, get the latest installer from [Github](https://github.com/bambulab/BambuStudio/releases) releases.
...
```bash
# Freecad is only in snap/flatpak, not in dnf
# both versions are not fresh. Currently 1.0.2
sudo snap install freecad
# flatpak install flathub org.freecad.FreeCAD
```

If instead of fresh install you upgrade the Distro, the FreeCAD might broke. Reinstall than.
...
# alternatively with MCAD libraries
sudo dnf install -y openscad-MCAD
```
## Fwupd
...
Still very shitty support in Linux.
