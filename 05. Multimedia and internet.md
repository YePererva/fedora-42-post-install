...
sudo dnf remove -y dragon totem showtime

# adding video player and multimedia editors
sudo dnf group install -y sound-and-video 
sudo dnf install -y vlc inkscape gimp -y

# MPV is nice alternative to VLC
...
sudo dnf install -y obs-studio obs-studio-plugin-x264 obs-studio-plugin-browser obs-studio-plugin-vlc-video
sudo dnf install -y ffmpeg-libs libva libva-utils
sudo dnf install -y libdvdcss opus libavcodec-freeworld 

# if using the NVidia cards
sudo dnf install -y libva-nvidia-driver

# audio codecs for noise supressiosn
sudo dnf install -y lv2-x42-plugins lsp-plugins-vst lsp-plugins
...
sudo dnf install yt-dlp ffmpeg --allowerasing -y
```
